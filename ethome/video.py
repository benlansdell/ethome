...
def _validate_metadata(metadata: dict, req_cols: list):
    ...
    for fn in metadata:
        n_dim_cols = sum(
            [
                x in metadata[fn].keys()
                for x in ["frame_width", "resolution", "frame_width_units"]
            ]
        )
        if (n_dim_cols > 0) and (n_dim_cols < 3):
            warnings.warn(
                "Must provide all of 'frame_width', 'resolution' and 'frame_width_units' to rescale. Not rescaling."
            )
        ...
        if n_dim_cols == 3:
            ...
            if not isinstance(metadata[fn]["frame_width"], numbers.Number):
                warnings.warn(
                    "'frame_width' must be a number to rescale. Not rescaling."
                )
                should_rescale = False
            if type(metadata[fn]["frame_width_units"]) is not str:
                warnings.warn(
                    "'frame_width_units' must be a string to rescale. Not rescaling."
                )
                should_rescale = False
            elif metadata[fn]["frame_width_units"].lower() not in UNIT_DICT:
                warnings.warn(
                    f"Units must be one of the following to rescale: {','.join(UNIT_DICT.keys())}. Not rescaling."
                )
                should_rescale = False
            ...
            if hasattr(metadata[fn]["resolution"], "__len__"):
                if len(metadata[fn]["resolution"]) != 2:
                    warnings.warn(
                        "'resolution' must be a list-like object of length 2 to rescale. Not rescaling."
                    )
                    should_rescale = False
            else:
                warnings.warn(
    ...
    for unit in unit_counts:
        if unit_counts[unit] > 0:
            if unit_counts[unit] != len(metadata):
                warnings.warn(
                    "Target units must be the same for all files. Not rescaling."
                )
                should_rescale = False
    ...
    if target_units not in UNIT_DICT:
        warnings.warn(
            f"Units must be one of the following: {','.join(UNIT_DICT.keys())}. Not rescaling."
        )
        should_rescale = False
    ...
    def add(
        ...
        **kwargs,
        ...
        if isinstance(feature_maker, types.FunctionType):
            CustomFeatureClass = feature_class_maker(
                "CustomFeatureClass", feature_maker, required_columns
            )
...
class EthologyMLAccessor(object):
    ...
    def _make_predefined_split(self, folds):  # pragma: no cover
        ...
        test_fold[test_indices == False] = np.argmin(
            folds[test_indices == False], axis=1
        )
...
class EthologyIOAccessor(object):
    ...
    def to_dlc_csv(self, base_dir: str, save_h5_too: bool = False) -> None:
        ...
        for fn in fns:
            ...
            selected_cols = []
            for animal, bp, component in product(
                df_all.pose.animals, df_all.pose.body_parts, ["x", "y", "likelihood"]
            ):
    ...
    def save_movie(
        ...
        for video in video_filenames:
            if "fps" not in self._obj.metadata.details[video]:
                raise ValueError(
                    f"FPS not in metadata for video {video}, skipping. Provide FPS in dataset creation."
                )
            rate = 1 / self._obj.metadata.details[video]["fps"]
            vid_in = self._obj.metadata.details[video]["video"]
            file_out = (
                os.path.splitext(os.path.basename(vid_in))[0]
                + "_".join(label_columns.keys())
                + ".mp4"
            )
            ...
            for idx, (col, print_label) in enumerate(label_columns.items()):
                ...
                behavior_pairs = _make_dense_values_into_pairs(
                    self._obj.loc[self._obj.filename == video, col], rate
                )
                this_label_string = ",".join(
                    [
                        f"drawtext=text='{print_label}':x=90:y={this_y_offset}:fontsize={font_size}:fontcolor={text_color}:enable='between(t,{pair[0]},{pair[1]})'"
...
def _load_nwb(
    ...
    col_names_old = None
    for fn in nwb_files:
        df_fn, body_parts, animals, col_names, scorer, meta = read_NWB_tracks(
            fn, part_renamer, animal_renamer
        )
        ...
        if col_names_old is not None:
            if col_names != col_names_old:
                raise RuntimeError(
                    "NWB files have different animals/body parts tracked. Must all be from same project"
                )
    ...
    df.pose.animal_setup = {
        "mouse_ids": animals,
        "bodypart_ids": body_parts,
        "colnames": col_names,
    }
...
def _load_tracks(
    ...
    for fn in df.metadata.details.keys():
        ...
        if ext == ".nwb":
            df_fn, body_parts, animals, col_names, scorer, _ = read_NWB_tracks(
                fn, part_renamer, animal_renamer
            )
        elif ext == ".csv":
            df_fn, body_parts, animals, col_names, scorer = read_DLC_tracks(
                fn, part_renamer, animal_renamer
            )
        else:
            ...
            try:
                ...
                df_fn, body_parts, animals, col_names, scorer = read_sleap_tracks(
                    fn, part_renamer, animal_renamer
                )
        ...
        if (
            rescale
            and ("frame_width_units" in df.metadata.details[fn])
            and ("frame_width" in df.metadata.details[fn])
            and ("resolution" in df.metadata.details[fn])
            and df.metadata.details[fn]["frame_width_units"].lower() in UNIT_DICT
        ):
            unit_scale_factor = UNIT_DICT[
                df.metadata.details[fn]["frame_width_units"].lower()
            ]
            ...
            df_fn["scale_factor"] = (
                df.metadata.details[fn]["frame_width"]
                / df.metadata.details[fn]["resolution"][1]
                * unit_scale_factor
            )
        else:
        ...
        if "fps" in df.metadata.details[fn]:
            df.metadata.details[fn]["duration"] = (n_rows) / df.metadata.details[fn][
                "fps"
            ]
        ...
        if col_names_old is not None:
            if col_names != col_names_old:
                raise RuntimeError(
                    "Tracking files have different columns. Must all be from same project"
                )
        col_names_old = col_names
    ...
    for animal in df.pose.animals:
        if animal in ["filename", "frame", "time", "label"]:
            warnings.warn(
                f"One of the animal names is protected ({animal}), unexpected behavior may occur."
            )
    return col_names
...
def _load_labels_boris(
    ...
    for vid in df.metadata.details:
        if "labels" in df.metadata.details[vid] and "fps" in df.metadata.details[vid]:
            ...
            print(
                f"'fps' not provided for video {vid}. Cannot link pose data to BORIS annotations."
            )
...
def get_sample_openfield_data():
    ...
    tracking_files = glob(
        os.path.join(cur_dir, "data", "dlc", "openfield", "*example.csv")
    )
    ...
    metadata = create_metadata(
        tracking_files, video=video, fps=fps, resolution=resolution
    )
...
def add_randomforest_predictions(df: pd.DataFrame):
    ...
    preds = cross_val_predict(
        model, df.ml.features, df.ml.labels, groups=df.ml.group, cv=cv
    )
    df["prediction"] = preds
