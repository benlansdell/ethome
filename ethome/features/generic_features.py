    ...
    for i, animal_i in enumerate(mouse_ids):
        ...
        for j, animal_j in enumerate(mouse_ids[:i]):
            ...
            features_df[f_new] = np.sqrt(
                (features_df[fx_i] - features_df[fx_j]) ** 2
...
def compute_centerofmass_interanimal_speed(
    ...
    for i, animal_i in enumerate(mouse_ids):
        ...
        for _, animal_j in enumerate(mouse_ids[:i]):
            ...
            vx_i = (
                _diff_within_group(features_df, "filename", fx_i, periods=n_shifts) / dt
            )
            vy_i = (
                _diff_within_group(features_df, "filename", fy_i, periods=n_shifts) / dt
            )
            vx_j = (
                _diff_within_group(features_df, "filename", fx_j, periods=n_shifts) / dt
            )
            vy_j = (
                _diff_within_group(features_df, "filename", fy_j, periods=n_shifts) / dt
            )
...
def compute_centerofmass(
    df: pd.DataFrame, raw_col_names: list, bodyparts: list = [], **kwargs
) -> pd.DataFrame:
...
def compute_centerofmass_velocity(
    ...
    for animal_id in mouse_ids:
        ...
        features_df[fy_new] = features_df[fys].sum(axis=1) / len(bodypart_ids)
        features_df[fx_new] = (
            _diff_within_group(features_df, "filename", fx_new, periods=n_shifts) / dt
        )
        features_df[fy_new] = (
            _diff_within_group(features_df, "filename", fy_new, periods=n_shifts) / dt
        )
...
def compute_part_velocity(
    ...
    for animal_id in mouse_ids:
        for bp in bodypart_ids:
            ...
            features_df[fx_new] = (
                _diff_within_group(features_df, "filename", fx, periods=n_shifts) / dt
            )
            features_df[fy_new] = (
                _diff_within_group(features_df, "filename", fy, periods=n_shifts) / dt
            )
        ...
        for j, bp2 in enumerate(bodypart_ids):
            if i < j:
                for mouse_id in mouse_ids:
                    ...
                    vx1 = (
                        _diff_within_group(
                            features_df, "filename", f1x, periods=n_shifts
                        )
                        / dt
                    )
                    vy1 = (
                        _diff_within_group(
                            features_df, "filename", f1y, periods=n_shifts
                        )
                        / dt
                    )
                    vx2 = (
                        _diff_within_group(
                            features_df, "filename", f2x, periods=n_shifts
                        )
                        / dt
                    )
                    vy2 = (
                        _diff_within_group(
                            features_df, "filename", f2y, periods=n_shifts
                        )
                        / dt
                    )
