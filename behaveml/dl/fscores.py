...
class macroF1(Callback):
    ...
    def on_epoch_end(self, epoch, logs):
        ...
        f1_val = f1_score(self.targets, pred, average="macro", labels = [0,1,2])
        print("val_f1: ", f1_val)
        print("full F1")
        print(f1_score(self.targets, pred, average=None))
...
class FBetaScore(tf.keras.metrics.Metric):
    ...
    def result(self):
        ...
        if self.average == "weighted":
            ...
            if self.labels:
                f1_score = tf.reduce_sum([f1_score[i] for i in self.labels])/len(self.labels)
            else:
...
class F1Score(FBetaScore):
    ...
    def __init__(
        ...
        super().__init__(num_classes, average, 1.0, threshold, name=name, dtype=dtype, labels = labels)
