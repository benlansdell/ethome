import pandas as pd 
from behaveml.dl.feature_engineering import make_features_mars_distr, \
...
def compute_distance_features(df : pd.DataFrame, raw_col_names : list, animal_setup : dict, **kwargs) -> pd.DataFrame:
    features_df, _, _ = make_features_distances(df[raw_col_names], animal_setup) 
    return features_df

def compute_velocity_features(df : pd.DataFrame, raw_col_names : list, animal_setup : dict, **kwargs) -> pd.DataFrame:
    features_df, _, _ = make_features_velocities(df[raw_col_names], animal_setup) 
    return features_df
