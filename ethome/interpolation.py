    ...
    for fn_in in edf.metadata.videos:
        ...
        for m in edf.pose.animals:
            for bp in edf.pose.body_parts:
                low_conf = (
                    df_filter_low_conf.loc[
                        edf.filename == fn_in, "_".join(["likelihood", m, bp])
                    ]
                    < conf_threshold
                )
                df_filter_low_conf.loc[
                    (edf.filename == fn_in) & low_conf, "_".join([m, "x", bp])
                ] = np.nan
                df_filter_low_conf.loc[
                    (edf.filename == fn_in) & low_conf, "_".join([m, "y", bp])
                ] = np.nan

        df_filter_low_conf.loc[(edf.filename == fn_in)] = df_filter_low_conf.loc[
            (edf.filename == fn_in)
        ].interpolate(axis=0, method="linear", limit_direction="both")

        if rolling_window:
            df_filter_low_conf.loc[
                (edf.filename == fn_in), edf.pose.raw_track_columns
            ] = (
                df_filter_low_conf.loc[
                    (edf.filename == fn_in), edf.pose.raw_track_columns
                ]
        ...
        df_filter_low_conf = df_filter_low_conf[
            edf.pose.raw_track_columns + ["filename", "frame"]
        ]
