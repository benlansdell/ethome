...
def interactive_tracks(
    ...
    all_x_cols = [
        f"{animal}_x_{bp}" for animal, bp in product(animals, df.pose.body_parts)
    ]
    all_y_cols = [
        f"{animal}_y_{bp}" for animal, bp in product(animals, df.pose.body_parts)
    ]
    ...
    interact(
        plot_frame, frame=IntSlider(min=start_frame, max=stop_frame, value=start_frame)
    )
...
def plot_unsupervised_results(
    ...
    **kwargs,
    ...
    for lb in all_labels:
        ...
        lab_bright = lab_col[0] * 0.299 + lab_col[1] * 0.587 + lab_col[2] * 0.114 > (
            120 / 255
        )
...
def plot_ethogram(
    ...
    plot_data = dataset.loc[dataset["filename"] == vid_key, query_label][
        :frame_limit
    ].to_numpy()
...
def create_sample_videos(
    ...
    for label_idx in labels:
        ...
        random_sample_indices = np.random.choice(
            behavior_rows.index,
            min(len(behavior_rows.index), N_supersample_rows),
            replace=False,
        )
        window_sizes = [get_window_size(label_idx, i) for i in random_sample_indices]
        window_sizes = sorted(window_sizes, key=lambda x: x[3], reverse=True)
        window_sizes = pd.DataFrame(
            window_sizes, columns=["start", "end", "pt", "len"]
        ).drop_duplicates(subset=["start", "end"])

        random_sample_indices = window_sizes["pt"].to_numpy()[:N_sample_rows]

        behavior_rows_sample = behavior_rows.loc[random_sample_indices].reset_index(
            drop=True
        )
        ...
        for r_idx, (vid_file, fn) in enumerate(zip(video_files, filenames)):
            behave_rows_sample_vid = behavior_rows_sample[
                behavior_rows_sample["filename"] == fn
            ]
            ...
            for idx in range(len(behave_rows_sample_vid)):
                ...
                out_file = os.path.join(
                    out_dir_vid, f"{vid_name}_second_{behavior_time}.avi"
                )
...
def create_mosaic_video(
    vid_dir: str, output_file: str, ndim: tuple = (1600, 1200)
) -> None:  # pragma: no cover
