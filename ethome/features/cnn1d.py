...
def build_baseline_model(
    ...
    def add_conv_bn_activate(model, out_dim, activation="relu", conv_size=3, drop=0.0):
    ...
    for ch in layer_channels:
        model = add_conv_bn_activate(model, ch, conv_size=conv_size, drop=dropout_rate)
...
class MABe_Generator:
    def __init__(
        ...
        for vc, key in enumerate(self.video_keys):
            ...
            self.X[key], _ = self.featurize(
                np.pad(pose_dict[key]["keypoints"], pad_width)
            )
        ...
        if self.mode == "predict":
            extra_predicts = (
                -len(self.indexes) % self.batch_size
            )  # So that last part is not missed
