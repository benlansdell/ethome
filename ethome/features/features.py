...
from ethome.features.generic_features import (
...
def feature_class_maker(
    ...
    def transform(self, edf: pd.DataFrame, **kwargs):
        ...
        if sum(checks) < len(self.required_columns):
            raise RuntimeError(
                f"DataFrame doesn't have necessary columns to compute this set of features. Requires columns: {self.required_columns}"
            )
        if edf[self.required_columns].isnull().values.any():
            warnings.warn(
                "Missing values in required data columns. May result in unexpected behavior. Consider interpolating or imputing missing data first."
            )
        new_features = self.feature_maker(
            edf, self.required_columns, **self.kwargs, **kwargs
        )
        return new_features
...
# Social mice studies
MARS = feature_class_maker(
    "MARSFeatures", compute_mars_features, default_tracking_columns
)
MARSReduced = feature_class_maker(
    "MARSReduced", compute_mars_reduced_features, default_tracking_columns
)
CNN1DProb = feature_class_maker(
    "CNN1DProb", compute_dl_probability_features, default_tracking_columns
)
Social = feature_class_maker(
    "Social", compute_social_features, default_tracking_columns
)
...
CentroidInteranimal = feature_class_maker(
    "CentroidInteranimal", compute_centerofmass_interanimal_distances
)

# Speeds between all animals' centroids
CentroidInteranimalSpeed = feature_class_maker(
    "CentroidInteranimalSpeed", compute_centerofmass_interanimal_speed
)
...
CentroidVelocity = feature_class_maker(
    "CentroidVelocity", compute_centerofmass_velocity
)
...
FEATURE_MAKERS_DESCRIPTION = [
    ...
    [
        "MARS Reduced",
        "mars_reduced",
        "MARS mouse resident-intruder features, reduced to 10 features",
    ],
    ["CNN1D Prob", "cnn1d_prob", "CNN1D probability features"],
    ["Social", "social", "Social features"],
    [
        "Centroids Interanimal",
        "centroids_interanimal",
        "Centroids interanimal features",
    ],
    [
        "Centroids Interanimal Speed",
        "centroids_interanimal_speed",
        "Centroids interanimal speed features",
    ],
    ...
    [
        "Intrabodypart Distances",
        "intrabodypartdistances",
        "Intrabodypart distances features",
    ],
]
