...
def test_dataset_creation_list(tracking_files):
    ...
    openfield_path = glob(
        os.path.join(
            cur_dir, "..", "ethome", "data", "dlc", "openfield", "*missingdata.csv"
        )
    )[0]

    nwb_path = get_sample_nwb_paths()
    dataset = create_dataset(
        [nwb_path, openfield_path], animal_renamer={"PoseEstimation": "ind1"}
    )
    assert type(dataset) is pd.DataFrame
...
def test_sample_simultaneous_behavior_data(tracking_files, simultaneous_labels):
    ...
    metadata = create_metadata(tracking_files[:1], labels=simultaneous_labels, fps=30)
...
def test_multiple_boris_behaviors():
    ...
    label_files = [
        os.path.join(
            ...
            "boris",
            "multiple_behaviors",
            "e3v813a-20210610T120637-121213_reencode_multiple_behaviors.csv",
        ),
        os.path.join(
            cur_dir,
            "..",
            ...
            "data",
            "boris",
            "e3v813a-20210610T121558-122141_reencode.csv",
        ),
    ]
    tracking_files = [
        os.path.join(
            ...
            "data",
            "dlc",
            "e3v813a-20210610T120637-121213DLC_dlcrnetms5_pilot_studySep24shuffle1_100000_el_filtered.csv",
        ),
        os.path.join(
            cur_dir,
            "..",
    ...
    metadata = create_metadata(tracking_files, labels=label_files, fps=fps)
...
def test_sample_singlemouse_data_missing():
    ...
    # Metadata is a dictionary that attaches each of the above parameters to the video/behavior annotations
    metadata = create_metadata(tracking_files, fps=fps, resolution=resolution)
...
def test_sample_threemouse_data():
    ...
    cur_dir = os.path.dirname(os.path.abspath(__file__))
    tracking_files = glob(
        os.path.join(cur_dir, "..", "ethome", "data", "dlc", "openfield", "*three.csv")
    )
    fps = 30  # (int) frames per second
    resolution = (480, 640)  # (tuple) HxW in pixels
    # Metadata is a dictionary that attaches each of the above parameters to the video/behavior annotations
