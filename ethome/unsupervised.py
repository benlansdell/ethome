...
def compute_tsne_embedding(
    ...
    random_indices = np.random.choice(
        tsne_data.shape[0], min(N_rows, tsne_data.shape[0]), replace=False
    )
    tsne_data = tsne_data[random_indices, :]
    tsne_embedding = TSNE(
        n_components=n_components, init="pca", perplexity=perplexity
    ).fit_transform(tsne_data)
    return tsne_embedding, random_indices


def compute_umap_embedding(
    dataset: pd.DataFrame, cols: list, N_rows: int = 20000, n_components=2, **kwargs
) -> np.ndarray:
    ...
    random_indices = np.random.choice(
        umap_data.shape[0], min(N_rows, umap_data.shape[0]), replace=False
    )
...
def compute_morlet(
    data: np.ndarray, dt: float = 1 / 30, n_freq: int = 5, w: float = 3
) -> np.ndarray:
...
def compute_density(
    ...
    if rows is not None:
        ...
        sample_rows = np.random.choice(
            dataset.index, min(N_sample_rows, len(dataset)), replace=False
        )
    print("Fitting KDE")
    den_est.fit(dataset.loc[sample_rows, ["embedding_0", "embedding_1"]])
    X_plot = np.array(
        np.meshgrid(np.linspace(xmin, xmax, n_pts), np.linspace(ymin, ymax, n_pts))
    ).T.reshape(-1, 2)
...
def compute_watershed(
    dens_matrix: np.ndarray, positive_only: bool = False, cutoff: float = 0
) -> tuple:
...
def cluster_behaviors(
    ...
    **Note that this will modify the dataset dataframe in place.**
    ...
    if N_rows is not None:
        random_indices = np.random.choice(
            embedding_data.shape[0], min(N_rows, embedding_data.shape[0]), replace=False
        )
    ...
    dens_matrix = compute_density(
        dataset, embedding_extent, bandwidth=bandwidth, n_pts=n_pts
    )
    print("Performing watershed transform of density")
    labels = compute_watershed(dens_matrix, positive_only=False, cutoff=0)

    dataset["embedding_index_0"] = dataset["embedding_0"].apply(
        lambda x: int(max(0, min(n_pts - 1, n_pts * (x - xmin) / (xmax - xmin))))
    )
    dataset["embedding_index_1"] = dataset["embedding_1"].apply(
        lambda x: int(max(0, min(n_pts - 1, n_pts * (x - ymin) / (ymax - ymin))))
    )
    dataset["unsup_behavior_label"] = dataset[
        ["embedding_index_0", "embedding_index_1"]
    ].apply(lambda x: labels[x[1], x[0]], axis=1)
